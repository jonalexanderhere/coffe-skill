...
const sidebarLinks = [
  ...
  { label: "Kelola User", href: "/admin", icon: Users },
  { label: "Kelola Kursus", href: "/admin", icon: BookOpen },
  { label: "Transaksi", href: "/admin", icon: DollarSign },
  { label: "Komentar", href: "/admin", icon: MessageSquare },
  { label: "Sertifikat", href: "/admin", icon: Award },
    ...
    <>
      <div className="flex items-center gap-2.5 px-4 py-5 border-b border-coffee-100 dark:border-charcoal-200">
        ...
        {!collapsed && (
          <div className="flex flex-col">
            ...
            <span className="text-[10px] font-semibold text-accent uppercase tracking-wider mt-1">Admin Panel</span>
          </div>
        )}
      </div>
      ...
      <nav className="flex-1 overflow-y-auto p-3 space-y-1">
        {sidebarLinks.map((link) => {
          const isActive = pathname === link.href && link.label === "Overview"; // simplification
          return (
            <Link
              key={link.label}
              href={link.href}
              ...
              className={`flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-colors ${
                isActive
                  ? "bg-coffee-800 text-white dark:bg-charcoal-300"
      ...
      <div className="p-3 border-t border-coffee-100 dark:border-charcoal-200">
        <button className="w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium text-red-500 hover:bg-red-50 dark:hover:bg-red-500/10 transition-colors">
        ...
        <header className="sticky top-0 z-30 bg-white/80 dark:bg-charcoal-light/80 backdrop-blur-md border-b border-coffee-100 dark:border-charcoal-200 px-4 lg:px-8 py-3">
          <div className="flex items-center gap-4">
            ...
            <div className="flex-1 max-w-md relative hidden sm:block">
              ...
              <input type="text" placeholder="Cari data admin..." className="w-full pl-10 pr-4 py-2 text-sm bg-coffee-50 dark:bg-charcoal border border-transparent rounded-xl text-coffee-800 dark:text-white focus:outline-none" />
            </div>
            <div className="ml-auto flex items-center gap-3">
              ...
              <div className="w-8 h-8 rounded-full bg-coffee-800 text-white flex items-center justify-center text-xs font-bold">
                AD
              </div>
            </div>
          </div>
